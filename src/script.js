...
let authorName = document.getElementById("auteur");
let generatorButton = document.getElementById("nouveau");

//index de la dernière citation affichée
let dernier = 0;

//Nombre aléatoire généré
let nombreAleatoire;

const genererCitation = (e) => {

  nombreAleatoire = genererNombreAleatoire();

  let citationSel = citations[nombreAleatoire];

  dernier = nombreAleatoire;

  citationElement.textContent = citationSel[0];
  authorName.textContent = citationSel[1];

}
generatorButton.addEventListener('click', genererCitation);
...
do {
nombreGen = genererNombreEntier(21);
}while(nombreGen === dernier);
